...
public:
    ...
    void swap_kth(int k){
        ...
        else if(length%2==1 && (length+1)/2==k)
            return;
        else{
            Node* first = get_nth(k-1);
            Node* last{ };

            if(length%2==0 && length/2==k){
                last = first->next; // consecutive

                link(first, last->next);
                link(first->prev, last);
                link(last, first);
            }
            else{
                last = get_nth(length - k); // NOT consecutive

                Node* first_prev = first->prev;
                Node* first_next = first->next;

                link(first,last->next);
                link(last->prev, first);
